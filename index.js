...
.setThumbnail()
.addField(`Chingle | Teşekkürler`, `**Selamlar, Ben Darwin (Chingle'nin Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Chingle | Prefix`, `**Chingle Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Chingle | Nasıl Kullanılır?`, `**Chingle botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`Chingle | Linkler`, `**Sohbet Kanalına !!davet Yazmanız Yeterlidir**`)
.setFooter(`Chingle | Gelişmiş Türkçe Bot | 2021`)
.setTimestamp();
...
var dm = client.channels.cache.get("846394705436737536")
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!713713727794446397>`) return msg.channel.send(`>Chingle **Prefix | Prefix**\n\n>   **Sanırım beni etiketlediniz.**\n >   Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`Chingle | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
